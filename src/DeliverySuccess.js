...
const mapStateToProps = (state) => {
    return {
        ...
        orders: state.orders ? state.orders.filter(order => order.status === 7): state.orders,
        orderGridData: state.orderGridData ? state.orderGridData.filter(order => order.status ===7) : state.orderGridData,
...
export default connect(mapStateToProps, actions)(DeliverySuccess);
